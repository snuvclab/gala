...
import os
import yaml
from deformer.lib.rotation_converter import batch_rodrigues, batch_axis2matrix
...
class SMPLX(nn.Module):
    ...
    def __init__(self, config):
        ...
        # hand components
        self.register_buffer('left_hand_components', torch.tensor(smplx_model.hands_componentsl[:6]))
        self.register_buffer('right_hand_components', torch.tensor(smplx_model.hands_componentsr[:6]))
        self.register_buffer('left_hand_mean', torch.tensor(smplx_model.hands_meanl))
        self.register_buffer('right_hand_mean', torch.tensor(smplx_model.hands_meanr))
    ...
    def forward(self, shape_params=None, expression_params=None,
                    ...
                    offset=None, transl=None, return_T = False, axis_pca=False, scale=None):
        """
            Args:
                ...
                jaw_pose: [N, 1, 3, 3]
                eye_pose: [N, 2, 3, 3]
                left_hand_pose: [N, 15, 3, 3] / PCA components
                right_hand_pose: [N, 15, 3, 3] / PCA components
            Returns:
        ...
        if axis_pca:
            ...
            global_pose = full_pose[:, :, :3].reshape(-1, 3)
            body_pose = full_pose[:, :, 3:66].reshape(-1, 3)
            jaw_pose = full_pose[:, :, 66:69].reshape(-1, 3)
            eye_pose = full_pose[:, :, 69:75].reshape(-1, 3)
            left_hand_pose = full_pose[:, :, 75:81].reshape(-1, 6)
            right_hand_pose = full_pose[:, :, 81:87].reshape(-1, 6)

            global_pose = batch_rodrigues(global_pose).reshape(batch_size, -1, 3, 3)
            body_pose = batch_rodrigues(body_pose).reshape(batch_size, -1, 3, 3)
            jaw_pose = batch_rodrigues(jaw_pose).reshape(batch_size, -1, 3, 3)
            eye_pose = batch_rodrigues(eye_pose).reshape(batch_size, -1, 3, 3)


            left_hand_pose = torch.einsum('bi,ij->bj', [left_hand_pose, self.left_hand_components])
            left_hand_pose = left_hand_pose + self.left_hand_mean.expand_as(left_hand_pose)
            right_hand_pose = torch.einsum('bi,ij->bj', [right_hand_pose, self.right_hand_components])
            right_hand_pose = right_hand_pose + self.right_hand_mean.expand_as(right_hand_pose)
            left_hand_pose = batch_rodrigues(left_hand_pose.reshape(-1, 3)).reshape(batch_size, -1, 3, 3)
            right_hand_pose = batch_rodrigues(right_hand_pose.reshape(-1, 3)).reshape(batch_size, -1, 3, 3)
