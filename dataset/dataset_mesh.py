...
class DatasetMesh(torch.utils.data.Dataset):
    ...
    def __init__(self, glctx, FLAGS, validate=False, gif=False):
        ...
        self.init_elevation_range_min= np.deg2rad(FLAGS.init_elevation_range[0])
        self.init_elevation_range_max= np.deg2rad(FLAGS.init_elevation_range[1])
    ...
    def _train_scene(self, itr):
        ...
        if ((itr+1)/self.FLAGS.batch) <=self.FLAGS.init_iter:
            rotate_x = -np.random.uniform(self.init_elevation_range_min, self.init_elevation_range_max)
        else:
        ...
        if ((itr+1)/self.FLAGS.batch) <=self.FLAGS.coarse_iter:
            if self.FLAGS.use_cam_normal:
                normal_rotate =  mv[:3, :3].T  # world to cam
            else:
    ...
    def train_scene_segm(i):
        fovy =  25.8
        proj_mtx = util.perspective(fovy, 1, 1, 50)
        rotate_x = np.deg2rad(-45)
        rotate_y = np.deg2rad(-180) +  (np.deg2rad(180) - np.deg2rad(-180)) * i / 18
        x, y, cam_radius = 0, 0, 3
        mv     = util.translate(x, y, -cam_radius) @ (util.rotate_x(rotate_x) @ util.rotate_y(rotate_y))
        mvp    = proj_mtx @ mv
        campos = torch.linalg.inv(mv)[:3, 3]
        rotate_y1 = 0    
        rotate_x1 = 0
        normal_rotate =  util.rotate_y_1(rotate_y1) @ util.rotate_x_1(rotate_x1)
        prompt_index = get_view_direction(thetas= rotate_x, phis = rotate_y, front= np.deg2rad(45))

        return {
            'mv' : mv[None, ...].cuda(),
            'mvp' : mvp[None, ...].cuda(),
            'campos' : campos[None, ...].cuda(),
            'resolution' : torch.tensor([512, 512]),
            'spp' : 1,
            'background': torch.zeros(1, 512, 512, 3, dtype=torch.float32, device='cuda'),
            'normal_rotate': normal_rotate[None,...].cuda(),
            'prompt_index' : prompt_index,
        }
    
    def train_scene_segm_2(i):
        fovy =  25.8
        proj_mtx = util.perspective(fovy, 1, 1, 50)
        rotate_x = 90 if i % 2 else -90
        rotate_y = np.deg2rad(-180) + (np.deg2rad(180) - np.deg2rad(-180)) * i / 4
        x, y, cam_radius = 0, 0, 3
        mv     = util.translate(x, y, -cam_radius) @ (util.rotate_x(rotate_x) @ util.rotate_y(rotate_y))
        mvp    = proj_mtx @ mv
        campos = torch.linalg.inv(mv)[:3, 3]
    ...
    def train_scene_body_zoom(i, cam_y):
        fovy =  30
        proj_mtx = util.perspective(fovy, 1, 1, 50)
        rotate_x = 0
        rotate_y = np.deg2rad(-180) + (np.deg2rad(180) - np.deg2rad(-180)) * i / 6
        x, y, cam_radius = 0, cam_y, 5
        mv     = util.translate(x, y, -cam_radius) @ (util.rotate_x(rotate_x) @ util.rotate_y(rotate_y))
        mvp    = proj_mtx @ mv
        campos = torch.linalg.inv(mv)[:3, 3]
        rotate_y1 = 0    
