...
class Texture2D(torch.nn.Module):
    ...
    def __init__(self, init, min_max=None):
        ...
        if isinstance(init, list):
            self.data = list(torch.nn.Parameter(mip.clone().detach(), requires_grad=False) for mip in init)
        elif len(init.shape) == 4:
            self.data = torch.nn.Parameter(init.clone().detach(), requires_grad=False)
        elif len(init.shape) == 3:
            self.data = torch.nn.Parameter(init[None, ...].clone().detach(), requires_grad=False)
        elif len(init.shape) == 1:
            self.data = torch.nn.Parameter(init[None, None, None, :].clone().detach(), requires_grad=False) # Convert constant to 1x1 tensor
        else:
    ...
    def sample(self, texc, texc_deriv, filter_mode='linear-mipmap-linear'):
        if isinstance(self.data, list):
            ...
            out = dr.texture(self.data, texc, texc_deriv, filter_mode='linear')
        return out
