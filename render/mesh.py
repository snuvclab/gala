...
######################################################################################
class Mesh:
    def __init__(self, v_pos=None, t_pos_idx=None, v_nrm=None, t_nrm=None, t_nrm_idx=None, v_tex=None, t_tex_idx=None, v_tng=None, t_tng_idx=None, material=None, base=None, f_seg=None, v_color=None):
        self.v_pos = v_pos           # Vertex positions [5344,3]
        self.v_nrm = v_nrm           # Vertex normals [5344,3]
        self.t_nrm = t_nrm           # Triangle normals [10688,3]
        self.v_tex = v_tex           # Vertex UV coordinates [5647,2] (The number is greater than the vertex count because a vertex may be part of multiple triangles with different texture coordinates)
        self.v_tng = v_tng           # Vertex tangents [5344,3]
        self.t_pos_idx = t_pos_idx   # Triangle faces, indices about the three vertices [10688,3]
        self.t_nrm_idx = t_nrm_idx   # Indices about which normal corresponds to each vertex in the triangle [10688,3]
        self.t_tex_idx = t_tex_idx   # Indices about which UV coordinates correspond to each vertex in the triangle [10688,3]
    ...
    def copy_none(self, other):
        ...
        if self.f_seg is None: # face segmentation label
            self.f_seg = other.f_seg
        if self.v_color is None: # vertex color
            self.v_color = other.v_color
        if self.t_nrm is None: # vertex color
    ...
    def clone(self):
        ...
        if out.f_seg is not None:
            out.f_seg = out.f_seg.clone().detach()
    ...
    def connected_faces(self):
        if self._connected_faces is None:
            from .nds.geometry import find_connected_faces
            self._connected_faces = find_connected_faces(self.t_pos_idx)
...
# Combine two meshes
######################################################################################
def combine(mesh1, mesh2, detach_mesh1=False):
    def modify_grad(grad):
        grad[torch.arange(len(mesh1.v_pos))] = 0
        handle.remove()
        return grad
    
    if detach_mesh1:
        handle = mesh1.v_pos.register_hook(lambda grad: modify_grad(grad))

    verts_comp = torch.cat([mesh1.v_pos, mesh2.v_pos], dim=0)
    faces_comp = torch.cat([mesh1.t_pos_idx, mesh2.t_pos_idx + len(mesh1.v_pos)], dim=0)
    
    imesh = Mesh(verts_comp, faces_comp,
                f_seg=torch.cat([torch.cat([torch.zeros_like(mesh1.t_pos_idx)[:,[0]], 
                                            torch.ones_like(mesh1.t_pos_idx)[:,[0]]], dim=-1),
                                torch.cat([torch.ones_like( mesh2.t_pos_idx)[:,[0]], 
                                            torch.zeros_like( mesh2.t_pos_idx)[:,[0]]], dim=-1)], 
                                dim=0))
...
# Combine two meshes
######################################################################################
def combine_texture(mesh1, mesh2, vcolor1, vcolor2, material, detach_mesh1=False):
    def modify_grad(grad):
        grad[torch.arange(len(vcolor1))] = 0
        handle.remove()
        return grad
    
    if detach_mesh1:
        handle = vcolor1.register_hook(lambda grad: modify_grad(grad))

    imesh = Mesh(v_pos=torch.cat([mesh1.v_pos, mesh2.v_pos], dim=0),
                            t_pos_idx=torch.cat([mesh1.t_pos_idx, mesh2.t_pos_idx + len(mesh1.v_pos)], dim=0),
                            material=material,
                            v_color=torch.cat([vcolor1, vcolor2], dim=0),
                            f_seg=torch.cat([torch.cat([torch.zeros_like(mesh1.t_pos_idx)[:,[0]], torch.ones_like(mesh1.t_pos_idx)[:,[0]]], dim=-1),
                                                torch.cat([torch.ones_like(mesh2.t_pos_idx)[:,[0]], torch.zeros_like(mesh2.t_pos_idx)[:,[0]]], dim=-1)], dim=0))
    imesh = auto_normals(imesh)
    return imesh
...
def load_mesh(filename, mtl_override=None, vcolor_filename=None):
    name, ext = os.path.splitext(filename)
    if ext == ".obj":
        return obj.load_obj(filename, clear_ks=True, mtl_override=mtl_override, vcolor_filename=vcolor_filename)
...
def auto_normals(imesh):
    ...
    return Mesh(v_nrm=v_nrm, t_nrm=util.safe_normalize(face_normals), t_nrm_idx=imesh.t_pos_idx, base=imesh)
