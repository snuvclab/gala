...
from utils.op_3d import OpenPoseDetectorRaw
from render import mesh as meshlib
...
def shade(
    ...
    ################################################################################
    perturbed_nrm = None
    if mode == 'appearance_modeling' and 'kd_ks_normal' in material:
        ...
        perturbed_nrm = None # layered decomposition
    ...
    elif mode == 'appearance_modeling' and gb_tangent.sum() != 0 and 'kd_ks_normal' not in material: # for GT mesh
        kd = material['kd'].sample(gb_texc, gb_texc_deriv)
        ks = torch.ones_like(gb_pos, dtype=torch.float32, device='cuda') * 0.4
    else:                                                                                            # for human & object mesh without uv textures
        kd =  torch.ones_like(gb_pos, dtype=torch.float32, device='cuda') * 0.4
        ks = torch.ones_like(gb_pos, dtype=torch.float32, device='cuda') * 0.4
    ...
    # 
    if if_use_bump == False or mode == 'geometry_modeling' or mode == 'refine':
        perturbed_nrm = None
    if if_normal:
        bsdf = 'normal'
    ...
    if mode == 'appearance_modeling': 
        # bsdf = 'diffuse' # albedo rendering for appearance optimization
        bsdf = 'kd'
    elif mode == 'geometry_modeling' or mode == 'refine':
        if type(bsdf) != str: bsdf = 'pbr'
   
    if bsdf == 'pbr':
        if mode == 'geometry_modeling' or mode == 'refine':
    ...
    buffers = {
        ...
        'segm'    : torch.cat((gb_seg, alpha), dim=-1),
        'v_color'    : torch.cat((gb_v_color, alpha), dim=-1),
...
def render_layer(
    ...
    if if_use_bump == False or mode == 'geometry_modeling':
        ...
        if mesh.v_tng is not None: # GT mesh
            gb_tangent, _ = interpolate(mesh.v_tng[None, ...], rast_out_s, mesh.t_tng_idx.int()) # Interpolate tangents
        else: # human, object mesh
            gb_tangent = torch.zeros_like(gb_normal)

    # segmentation rendering
    if mesh.f_seg is not None:
        gb_seg, _ = interpolate(mesh.f_seg[None, ...].float(), rast_out_s, face_normal_indices.int())
    else:
        gb_seg = torch.zeros_like(gb_pos)[..., [0]]

    # vertex color rendering
    if mesh.v_color is not None:
        gb_v_color, _ = interpolate(mesh.v_color[None, ...].float(), rast_out_s, mesh.t_pos_idx.int())
    else:
        gb_v_color = torch.zeros_like(gb_pos)

    # 
    # Texture coordinate
    # assert mesh.v_tex is not None
    if mesh.v_tex is not None and mode == 'appearance_modeling':
    ...
    # Shade
    ################################################################################
    buffers = shade(gb_pos, gb_geometric_normal, gb_normal, gb_tangent, gb_texc, gb_texc_deriv, gb_seg, gb_v_color,
...
def render_mesh(
    ...
    def composite_buffer(key, layers, background, antialias):
        ...
        for buffers, rast in reversed(layers):
            ...
            accum = torch.lerp(accum, torch.cat((buffers[key][..., :-1], torch.ones_like(buffers[key][..., -1:])), dim=-1).float(), alpha) #[1,512,512,4] 最后一个通道是alpha通道，若像素有物体则为1，无物体则为0  outi=starti+weighti×(endi−starti)
            if antialias:
    ...
    if op_3d is not None:
        op_3d = op_3d[None].to(mtx_in.device)

    v_pos = mesh.v_pos
    # Object zoom in
    if zoom_in_params is not None:
        scale = zoom_in_params['scale']
        transl = zoom_in_params['transl']
        v_pos = v_pos - transl
        v_pos = v_pos / scale
        if op_3d is not None:
            op_3d = op_3d - transl
    ...
    if op_3d is not None:
        op_3d_clip_homo = ru.xfm_points(op_3d, mtx_in).cpu()
        op_3d_clip = op_3d_clip_homo[...,:3] / op_3d_clip_homo[...,3:]
        op_img = [OpenPoseDetectorRaw.draw_bodypose(op_3d_rot[:, :2])[None] for op_3d_rot in op_3d_clip]  # multi-view openpose images
        op_img = torch.cat(op_img)
    t_pos_idx = mesh.t_pos_idx.cuda()

    mesh_copy = meshlib.Mesh(base=mesh, v_pos=v_pos, t_pos_idx=t_pos_idx)

    # Render all layers front-to-back
    layers = []
    with dr.DepthPeeler(ctx, v_pos_clip, t_pos_idx.int(), full_res) as peeler:
        for _ in range(num_layers):
            rast, db = peeler.rasterize_next_layer()
            layers += [(render_layer(rast, db, mesh_copy, view_pos, lgt, resolution, spp, msaa, bsdf, if_normal, normal_rotate, mode, if_flip_the_normal, if_use_bump), rast)]
    ...
    if op_3d is not None:
        out_buffers['op_img'] = op_img.to(torch.float32)

    if return_rast: return rast[..., -1] # return face indices
    else: return out_buffers
